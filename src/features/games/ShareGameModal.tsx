...
import type { AdminProfessionalRow, ProfessionalDirectoryRow } from "@/lib/laravel-api";
...
export function ShareGameModal({ open, onOpenChange, mode, gameType, gameId, title }: Props): JSX.Element {
    ...
    (async () => {
      ...
      try {
        const [pros, shares] = await Promise.all([
          ...
          mode === "admin" ? api.adminGetGameShares(gameType, gameId) : api.professionalGetGameShares(gameType, gameId),
        ]);
        if (cancelled) return;
        const myId = auth.user?.id ?? 0;
        setProfessionals((pros ?? []).filter((p: any) => (p?.id ?? 0) !== myId) as any);
        setSelectedIds((shares.professional_ids ?? []).filter((id) => id !== myId));
      } catch {
        if (!cancelled) toast({ title: "Compartilhar", description: "Não foi possível carregar profissionais.", variant: "destructive" });
      } finally {
  ...
  }, [open, mode, gameType, gameId, toast, auth.user?.id]);
  ...
  const onSave = async () => {
    setSaving(true);
    try {
      if (mode === "admin") await api.adminShareGame(gameType, gameId, selectedIds);
      else await api.professionalShareGame(gameType, gameId, selectedIds);
      toast({ title: "Compartilhar", description: "Jogo compartilhado com sucesso." });
      onOpenChange(false);
    } catch {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        ...
        <div className="text-sm text-muted-foreground">
          Compartilhar <span className="font-semibold text-foreground">“{title}”</span> com outros profissionais.
          <div className="mt-1">Isso não altera pacientes atribuídos (não duplica envios).</div>
        </div>
