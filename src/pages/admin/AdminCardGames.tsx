...
import { Layers, Edit, Trash2, Search, Play, Share2 } from "lucide-react";
...
export default function AdminCardGames() {
  ...
  const [shareTarget, setShareTarget] = useState<CardGameRow | null>(null);
  ...
  const refresh = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Não foi possível carregar a lista do Jogo das Cartas.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const groupedByUser = useMemo(() => {
    const byUser = new Map<number, CardGameRow[]>();
    ...
    <div className="min-h-full py-8 lg:py-12">
      <div className="container mx-auto px-4">
                      ...
                      {list.map((g) => (
                        ...
                        >
                          <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                            <div className="w-12 h-12 rounded-xl bg-brand-brown/10 flex items-center justify-center flex-shrink-0 overflow-hidden">
                              {g.background_url ? (
                                <img src={normalizeMediaUrl(g.background_url)} alt="" className="w-full h-full object-cover" />
                              ) : (
                                <Layers size={22} className="text-brand-brown" />
                              )}
                            </div>
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="text-sm text-muted-foreground mt-2">{g.cards_count} carta(s) no baralho</div>
                            </div>
                            ...
                            <div className="flex items-center gap-2 flex-wrap justify-end">
                              <Button
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  navigate(`${base}/jogos/cartas/${g.id}/editar`);
                                }}
        ...
        <BrandedConfirmDialog
          ...
          title="Excluir jogo?"
          description={deleteTarget ? `Excluir "${deleteTarget.title}"? Esta ação é permanente.` : "Esta ação é permanente."}
