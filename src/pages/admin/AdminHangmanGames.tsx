...
import { Type, Plus, Search, Trash2, Pencil, Play, Image as ImageIcon, Share2 } from "lucide-react";
...
export default function AdminHangmanGames() {
  ...
  const [shareTarget, setShareTarget] = useState<HangmanGameRow | null>(null);
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return games;
    return games.filter((g) => g.title.toLowerCase().includes(q) || g.description.toLowerCase().includes(q));
  }, [games, search]);

  const groupedByUser = useMemo(() => {
    const byUser = new Map<number, HangmanGameRow[]>();
                            ...
                            <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                              <div className="w-12 h-12 rounded-xl bg-brand-orange/10 flex items-center justify-center flex-shrink-0 overflow-hidden">
                                {g.thumbnail?.url ? (
                                  ...
                                  <ImageIcon size={24} className="text-brand-orange" />
                                )}
                              </div>
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex flex-wrap items-center gap-2 mb-1">
                                  ...
                                  <span className="text-xs px-2 py-0.5 rounded-full bg-muted text-muted-foreground">
                                    {g.word_length} letras
                                  </span>
                                  <span className="text-xs px-2 py-0.5 rounded-full bg-muted text-muted-foreground">
                                    {g.support_images?.length ?? 0} imagem(ns)
                                  </span>
                                </div>
