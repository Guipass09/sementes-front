...
import { Plus, Upload, X, ArrowLeft, Grid3X3, User as UserIcon } from "lucide-react";
...
export default function AdminMemoryGameCreate() {
  ...
  const [pairs, setPairs] = useState<PairInput[]>(() => Array.from({ length: 6 }, () => ({ file: null, previewUrl: null })));
  ...
  // Ajusta pares quando pairsCount muda
  useEffect(() => {
  ...
  const missingImages = useMemo(() => pairs.some((p) => !p.file), [pairs]);

  const toggleUser = (id: number) => {
    setSelectedUserIds((prev) => (prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]));
  };

  const setPairFile = (idx: number, file: File | null) => {
    setPairs((prev) => {
      const copy = [...prev];
      const old = copy[idx];
      if (!old) return prev;
  ...
  const onSubmit = async () => {
    ...
    if (missingImages) {
      toast({ title: "Faltam imagens", description: "Envie 1 imagem para cada par.", variant: "destructive" });
      return;
    }

    const files = pairs.slice(0, finalPairsCount).map((p) => p.file!).filter(Boolean);
    if (files.length !== finalPairsCount) {
      toast({ title: "Imagens incompletas", description: "Envie 1 imagem para cada par.", variant: "destructive" });
      return;
    }

    setSaving(true);
    try {
      const created = await (auth.user?.role === "professional" ? api.professionalCreateMemoryGame : api.adminCreateMemoryGame)({
        title: title.trim(),
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="inline-flex items-center gap-2 text-sm text-muted-foreground mb-2">
                ...
                Novo recurso: Jogo da Memória
              </div>
              <h1 className="text-2xl lg:text-3xl font-display font-bold text-foreground">
                Criar Jogo
              </h1>
              <p className="text-muted-foreground">
                Envie imagens e o sistema monta os pares automaticamente.
              </p>
            </div>
          ...
          <div className="lg:col-span-7 bg-card rounded-2xl border border-border p-6 shadow-sm">
            <div className="space-y-5">
              <div className="space-y-2">
                ...
                <Input id="title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Ex: Ache os pares das frutas" />
              </div>
              ...
              <div className="pt-2">
                <div className="flex items-center justify-between mb-2">
                  <div className="font-semibold text-foreground">Imagens (1 por par)</div>
                  ...
                  {pairs.map((p, idx) => (
                    <div key={idx} className="rounded-xl border border-border bg-background/50 p-3">
                      <div className="aspect-square rounded-lg bg-muted/30 overflow-hidden flex items-center justify-center">
                        {p.previewUrl ? (
                          <img src={p.previewUrl} alt={`Par ${idx + 1}`} className="w-full h-full object-cover" />
                        ) : (
                          <div className="text-center text-muted-foreground">
                            <Upload className="h-6 w-6 mx-auto mb-1" />
                            <div className="text-xs">Par {idx + 1}</div>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="mt-3 flex items-center gap-2">
                        <div className="flex-1">
                          <input
                            ...
                            accept="image/*"
                            className="sr-only"
                            onChange={(e) => setPairFile(idx, e.target.files?.[0] ?? null)}
                          />
                          <Button asChild type="button" variant="secondary" className="w-full cursor-pointer">
                            <label htmlFor={`pair-image-${idx}`}>{p.file ? "Trocar" : "Enviar"}</label>
                          </Button>
                        </div>
                        <Button
                          type="button"
