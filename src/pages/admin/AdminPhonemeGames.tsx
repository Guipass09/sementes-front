...
import { Gamepad2, Plus, Search, Trash2, Pencil, Image as ImageIcon, Play, Share2 } from "lucide-react";
...
export default function AdminPhonemeGames() {
  ...
  const [games, setGames] = useState<PhonemeGameRow[]>([]);
  const [users, setUsers] = useState<Array<{ id: number; name: string; email: string; profile_photo_url?: string | null }>>([]);
  const [search, setSearch] = useState("");
  const [deleteOpen, setDeleteOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<PhonemeGameRow | null>(null);
  const [shareOpen, setShareOpen] = useState(false);
  const [shareTarget, setShareTarget] = useState<PhonemeGameRow | null>(null);
  ...
  const refresh = async () => {
    ...
    try {
      const [res, u] = await Promise.all([
        isProfessional ? api.professionalListPhonemeGames() : api.adminListPhonemeGames(),
  ...
  const groupedByUser = useMemo(() => {
    const byUser = new Map<number, PhonemeGameRow[]>();
  ...
  const confirmDelete = async () => {
    ...
    try {
      await (isProfessional ? api.professionalDeletePhonemeGame(deleteTarget.id) : api.adminDeletePhonemeGame(deleteTarget.id));
    ...
    <div className="min-h-full py-8 lg:py-12">
      <div className="container mx-auto px-4">
        <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <Button onClick={() => navigate(`${base}/jogos/fonema/novo`)} className="w-full sm:w-auto">
                        ...
                        {list.map((g) => (
                          ...
                          >
                            <div className="flex items-center gap-3 min-w-0 flex-1">
                              <div className="h-12 w-12 rounded-xl bg-brand-purple/10 overflow-hidden flex items-center justify-center">
                                {g.thumbnail?.left_url ? (
                                  <img src={normalizeMediaUrl(g.thumbnail.left_url)} alt="" className="w-full h-full object-cover" />
                                ) : (
                              ...
                              <div className="min-w-0">
                                ...
                                <div className="text-xs text-muted-foreground mt-1">{g.sessions_count} sessão(ões)</div>
                              </div>
                            </div>

                            <div className="flex flex-wrap gap-2 justify-end">
                              <Button variant="outline" onClick={() => navigate(`/jogos/fonema/${g.id}`)}>
                              ...
                              <Button
                                variant="outline"
                                onClick={() => navigate(`${base}/jogos/fonema/${g.id}/editar`)}
                                disabled={isProfessional && (g.created_by?.id ?? 0) !== myId}
                              >
